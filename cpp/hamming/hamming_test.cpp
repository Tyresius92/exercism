...
BOOST_AUTO_TEST_CASE(domain_error_when_first_string_invalid)
{
        BOOST_REQUIRE_THROW(hamming::compute("AZA", "AAA"), std::domain_error);
}

BOOST_AUTO_TEST_CASE(domain_error_when_second_string_invalid)
{
        BOOST_REQUIRE_THROW(hamming::compute("AAA", "AAZ"), std::domain_error);
}